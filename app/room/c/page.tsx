...
export default function CZonePage() {
  // 상태 관리
  const [currentImage, setCurrentImage] = useState(0)
  const [sliderImages, setSliderImages] = useState<string[]>([])
  const [cRooms, setCRooms] = useState<Room[]>([])
  const [cSummary, setCSummary] = useState<Summary | null>(null)
              ...
              <div className="container mx-auto px-4 md:px-8">
                <div className="flex items-center justify-center py-3 md:py-4">
                  <div className="flex items-center space-x-2 md:space-x-8 text-base md:text-xl">
                    ...
                    <span className="text-black">|</span>  
                    <Link href="/room/c" className="text-black hover:text-gray-700 cursor-pointer font-bold">
                      C동
                    </Link>
                    <span className="text-black">|</span>  
        ...
        <div className="py-10 md:py-20 bg-gray-50">
          <div className="container mx-auto px-4 md:px-8">
            <div className="max-w-6xl mx-auto">
              {/* 헤더 - DB 데이터 사용 with whitespace-pre-wrap */}
              ...
              {/* C Zone Overview와 Information을 좌우로 배치 */}
              <div className="grid grid-cols-2 gap-4 md:gap-16 mb-8 md:mb-16">
                {/* 왼쪽 전체 영역 - C Zone Overview */}
                <div className="grid grid-cols-2 gap-2 md:gap-8">
                  {/* 왼쪽 - 제목 */}
                  <div>
                    <h3 className="text-[11px] md:text-lg font-medium text-black">C Zone</h3>
                  ...
                  <div className="space-y-1 md:space-y-3 text-[10px] md:text-base">
                    <div>
                      ...
                      <span className="text-black">• {cSummary?.areaRange || '로딩중...'}</span>
                    </div>
                    <div>
                      <span className="text-black">기준인원</span><br />
                      <span className="text-black">• {cSummary?.standardCapacityRange || '로딩중...'}</span>
                    </div>
                    <div>
                      <span className="text-black">최대인원</span><br />
                      <span className="text-black">• {cSummary?.maxCapacityRange || '로딩중...'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 md:gap-8">
                  ...
                  <div className="space-y-1 md:space-y-3 text-[10px] md:text-base">
                    ...
                    <div>
                      ...
                      <span className="whitespace-pre-wrap break-words text-black">• {getContent('info_pet')?.content || '가능'}</span>
                    </div>
                    <div>
                      <span className="text-black">수영장</span><br />
                      <span className="whitespace-pre-wrap break-words text-black">• {getContent('info_pool')?.content || '실내수영장'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="block md:hidden">
                <div className="grid grid-cols-3 gap-2">
                  {cRooms.map((room) => (
                    <a key={room.id} href={`/room/c/${room.id.toLowerCase()}`} className="block">
                      <button 
              ...
              {/* 데스크톱: 객실 선택 버튼들 - 동적으로 생성 (원본 유지) */}
              <div className="hidden md:flex flex-col gap-8">
                {/* 4개씩 묶어서 표시 */}
                {Array.from({ length: Math.ceil(cRooms.length / 4) }, (_, rowIndex) => (
                  <div key={rowIndex} className={`flex ${rowIndex === Math.ceil(cRooms.length / 4) - 1 && cRooms.length % 4 === 1 ? 'justify-start w-full' : 'justify-center'} gap-12`}>
                    {cRooms.slice(rowIndex * 4, rowIndex * 4 + 4).map((room, index) => (
                      <a 
                        key={room.id} 
                        href={`/room/c/${room.id.toLowerCase()}`} 
                        className="block"
                        style={rowIndex === Math.ceil(cRooms.length / 4) - 1 && cRooms.length % 4 === 1 && index === 0 ? { marginLeft: 'calc(50% - 468px)' } : {}}
                      >
                        <button 
