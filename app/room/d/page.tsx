              ...
              <div className="container mx-auto px-4 md:px-8">
                <div className="flex items-center justify-center py-3 md:py-4">
                  <div className="flex items-center space-x-2 md:space-x-8 text-base md:text-xl">
                    ...
                    <span className="text-black">|</span>  
                    <Link href="/room/d" className="text-black hover:text-gray-700 cursor-pointer font-bold">
                      D동
                    </Link>  
                  </div>
                </div>
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-16 mb-8 md:mb-16">
                ...
                <div className="grid grid-cols-2 gap-2 md:gap-8">
                  {/* 왼쪽 - 제목 */}
                  <div>
                    <h3 className="text-[11px] md:text-lg font-medium text-black">D Zone</h3>
                  ...
                  <div className="space-y-1 md:space-y-3 text-[10px] md:text-base">
                    <div>
                      ...
                      <span className="text-black">• {dSummary?.areaRange || '로딩중...'}</span>
                    </div>
                    <div>
                      <span className="text-black">기준인원</span><br />
                      <span className="text-black">• {dSummary?.standardCapacityRange || '로딩중...'}</span>
                    </div>
                    <div>
                      <span className="text-black">최대인원</span><br />
                      <span className="text-black">• {dSummary?.maxCapacityRange || '로딩중...'}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="block md:hidden">
                <div className="grid grid-cols-3 gap-2">
                  {dRooms.map((room) => (
                    <a key={room.id} href={`/room/d/${room.id.toLowerCase()}`} className="block">
                      <button 
              ...
              <div className="hidden md:flex flex-col gap-8">
                {/* 4개씩 묶어서 표시 */}
                {Array.from({ length: Math.ceil(dRooms.length / 4) }, (_, rowIndex) => {
                  const rowRooms = dRooms.slice(rowIndex * 4, rowIndex * 4 + 4)
                  const isLastRow = rowIndex === Math.ceil(dRooms.length / 4) - 1
                  const remainingCount = dRooms.length % 4
                  const shouldLeftAlign = isLastRow && remainingCount > 0 && remainingCount < 4
                  
                  return (
                    <div 
                      key={rowIndex} 
                      className={`flex ${shouldLeftAlign ? 'justify-start w-full' : 'justify-center'} gap-12`}
                      style={shouldLeftAlign ? { paddingLeft: 'calc(50% - 468px)' } : {}}
                    >
                      {rowRooms.map((room) => (
                        <a key={room.id} href={`/room/d/${room.id.toLowerCase()}`} className="block">
                          <button 
